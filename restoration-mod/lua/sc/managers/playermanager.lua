...
function PlayerManager:get_max_grenades(grenade_id)
	...
	max_amount = max_amount + self:get_equipped_armor_grenade_add() or 0
...
function PlayerManager:_internal_load()
	...
	amount = amount + self:get_equipped_armor_grenade_add()
...
function PlayerManager:activate_temporary_upgrade(category, upgrade)
	...
	if upgrade == "doctor_bag_health_regen" then
		local doctor_armor = self:get_equipped_armor_medic()
		time = time + doctor_armor.time
	end
...
-- 检测护甲以及目前手持手雷可增加的数量
function PlayerManager:get_equipped_armor_grenade_add()
	local grenade_id = managers.blackmarket:equipped_grenade()
	if tweak_data.blackmarket.projectiles[grenade_id].ability then
		return 0
	end
	...
	local equipped_armor = managers.blackmarket:equipped_armor()
	local armor_level = tweak_data.blackmarket.armors[equipped_armor].upgrade_level
	local throwable_add = tweak_data.upgrades.values.player.body_armor.throwable_add
	
	if throwable_add[armor_level] ~= 0 then
		for _, amount in ipairs(throwable_add.amount) do
			if max_amount <= amount[1] then
				add_amount = amount[2]
				break
			end
		end
	...
	return add_amount or 0
end

-- 检测护甲以及目前大包和小包可增加的血量以及时间
function PlayerManager:get_equipped_armor_medic()
	local add_health_doc = {
		health_add = 1,
		time = 0
	}
	
	local add_health_fak = 0
	local equipped_armor = managers.blackmarket:equipped_armor()
	local armor_level = tweak_data.blackmarket.armors[equipped_armor].upgrade_level
	local medic_armor = tweak_data.upgrades.values.player.body_armor.medic_armor
	
	if medic_armor[armor_level] ~= 0 then
		-- doctor bag
		if medic_armor[armor_level] == 1 or medic_armor[armor_level] == 3 then
			add_health_doc.health_add = medic_armor.medic_bag[1]
			add_health_doc.time = medic_armor.medic_bag[2]
		end
		
		-- first aid kit
		if medic_armor[armor_level] == 2 or medic_armor[armor_level] == 3 then
			for _, health in ipairs(medic_armor.first_aid) do
				add_health_fak = add_health_fak + health * 0.1
			end
		end
	end

	return add_health_doc, add_health_fak
end

function PlayerManager:get_equipped_armor_recoil()
	local recoil = {1, 1, 1}
	local equipped_armor = managers.blackmarket:equipped_armor()
	local armor_level = tweak_data.blackmarket.armors[equipped_armor].upgrade_level
	local stability_increase = tweak_data.upgrades.values.player.body_armor.stability_increase
	
	if stability_increase[armor_level] ~= 0 then
		recoil = {
			stability_increase.state[1],
			stability_increase.state[2],
			stability_increase.state[3]
		}
	end
...
--PP玩具枪
Hooks:PostHook(PlayerManager, "init", "PlayerManager_Wpn138Max", function(self)
	self._all_skill_upgrades_138 = {}

	for _, trees in pairs(tweak_data.skilltree.trees) do
		for _, tier in ipairs(trees.tiers) do
			for _, skill in ipairs(tier) do
				for i = 1 ,2 do
					for _, v in pairs(tweak_data.skilltree.skills[skill][i]) do
						if type(v) == "table" then
							for _, skill_upg in ipairs(v) do
								local definitions = tweak_data.upgrades.definitions
								if definitions[skill_upg] and definitions[skill_upg].upgrade and definitions[skill_upg].upgrade.upgrade then
									local upgrade = definitions[skill_upg].upgrade
									self._all_skill_upgrades_138[upgrade.category] = self._all_skill_upgrades_138[upgrade.category] or {}
									self._all_skill_upgrades_138[upgrade.category][upgrade.upgrade] = math.max(upgrade.value, self._all_skill_upgrades_138[upgrade.category][upgrade.upgrade] or 0)
								end
							end
						end
					end
...
local PlayerManager_has_category_upgrade = PlayerManager.has_category_upgrade
function PlayerManager:has_category_upgrade(category, upgrade, ...)
	if self:wpn_is_138_max() then
		if not self._all_skill_upgrades_138[category] then
			return PlayerManager_has_category_upgrade(self, category, upgrade, ...)
		end

		if not self._all_skill_upgrades_138[category][upgrade] then
			return PlayerManager_has_category_upgrade(self, category, upgrade, ...)
		end
...
function PlayerManager:upgrade_value(category, upgrade, default, ...)
	if self:wpn_is_138_max()  then
		...
		local level = self._all_skill_upgrades_138[category][upgrade]
		if tweak_data.upgrades.values[category] and tweak_data.upgrades.values[category][upgrade] and tweak_data.upgrades.values[category][upgrade][level] then
...
local PlayerManager_team_upgrade_value = PlayerManager.team_upgrade_value
function PlayerManager:team_upgrade_value(category, upgrade, default, ...)
	if self:wpn_is_138_max()  then
		...
		if not self._all_skill_upgrades_138[category] then
			return PlayerManager_team_upgrade_value(self, category, upgrade, default, ...)
		end
...
local PlayerManager_upgrade_level = PlayerManager.upgrade_level
function PlayerManager:upgrade_level(category, upgrade, default, ...)
	if self:wpn_is_138_max()  then
		if not self._all_skill_upgrades_138[category] then
			return PlayerManager_upgrade_level(self, category, upgrade, default, ...)
		end
...
function PlayerManager:upgrade_level_nil(category, upgrade)
	if self:wpn_is_138_max()  then
		return self._all_skill_upgrades_138[category][upgrade]
	end
	
	return self._global.upgrades[category] and self._global.upgrades[category][upgrade]
end

function PlayerManager:wpn_is_138_max()
	local blackmarket = managers.blackmarket
	if blackmarket then
		local v = 0
		local primary_weapon = blackmarket:equipped_primary() and blackmarket:equipped_primary().weapon_id
		local secondary_weapon = blackmarket:equipped_secondary() and blackmarket:equipped_secondary().weapon_id
		local equipped_melee_weapon = blackmarket:equipped_melee_weapon() and blackmarket:equipped_melee_weapon()
		if primary_weapon and primary_weapon == "toym16" then
			v=v+1
		end
		
		if secondary_weapon and secondary_weapon == "toy1911" then
			v=v+1
		end
		
		if equipped_melee_weapon and equipped_melee_weapon == "toyknife" then
			v=v+1
		end
		
		if v == 3 or Global.skilltree_manager.skills["juggernaut"].unlocked >= 1 then
			if not Global.blackmarket_manager.armors["level_7"].unlocked then
				managers.blackmarket:on_aquired_armor(tweak_data.upgrades.definitions["body_armor6"], "level_7", true)
			end
		else
			if Global.blackmarket_manager.armors["level_7"].unlocked then
				managers.blackmarket:on_unaquired_armor(tweak_data.upgrades.definitions["body_armor6"], "level_7")
			end
		end
