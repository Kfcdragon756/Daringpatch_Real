...
function BlackMarketGui:update_info_text()
			...
			if category == slot_data.category then
				...
				if custom_stats then
					for part_id, stats in pairs(custom_stats) do
						...
						-- [GPT合并1] 冲突1：12.4 增加了 ignore_bullet_class 检查，ybspatch 这里原本没有；本次保留 12.4 条件并继续保留后续 ybspatch 自定义弹药识别。
						--下面的部分应该是添加用于检测一些东西的吧
						if stats.bullet_class == "InstantExplosiveBulletBase" and not stats.ignore_bullet_class then
							exp_ammo = true
						end
						if stats.bullet_class_cus == "BuckShotPierceDaring" then
							buck_daring = true
						end
				...
				if weapon_tweak then
					...
					-- [GPT合并1] 冲突2：12.4 简化/新增了爆头倍率描述分支，ybspatch 扩展了多弹丸、爆头倍率与物体伤害描述；本次保留 ybspatch 扩展块，其英文回退逻辑中已覆盖 12.4 的基础爆头倍率描述。
				if weapon_tweak and weapon_tweak.alt_shotgunraycast and rays > 1 then
					...
					if buck_daring then
						description = description .. "\n" .. managers.localization:text("bm_menu_buck_daring")
					end
					--[[if object_sasha_daring then
						description = description .. "\n" .. managers.localization:text("bm_sasha_object_damage_daring")
					end--]]
					for color_id in string.gmatch(description, "#%{(.-)%}#") do
						table.insert(updated_texts[4].resource_color, tweak_data.screen_colors[color_id])
					end
				...
				-- 榴弹衰减曲线描述
				local has_exp_falloff_desc = weapon_tweak and weapon_tweak.exp_falloff_desc
				...
				if sms < 1 then  --此处可以考虑优化，待定；[GPT合并1] 修复此前误拼接为爆头倍率文本的问题，并移除重复描述块
	...
	elseif identifier == self.identifiers.grenade then
		-- [GPT合并1] 冲突3：12.4 新增冷却类投掷物不吃 throwables_multiplier 的判断；ybspatch 新增护甲投掷物加成。这里合并二者。
		...
		local equipped_armor_tweak = equipped_armor and tweak_data.blackmarket.armors[equipped_armor]
		local equipped_armor_level = equipped_armor_tweak and equipped_armor_tweak.upgrade_level
		local equipped_armor_upgrade = equipped_armor_level and managers.player:body_armor_value("throwable_add", equipped_armor_level, 0) or 0
		local armor_upgrade_amount = tweak_data.upgrades.values.player.body_armor.throwable_add.amount
		local Metro_max_amount = tweak_data.blackmarket.projectiles[slot_data.name].max_amount
		local Metro_add_amount = 0
		if equipped_armor_upgrade and equipped_armor_upgrade == 1 then
			for _, amount in ipairs(armor_upgrade_amount) do
				if Metro_max_amount <= amount[1] then
					Metro_add_amount = amount[2]
					break
				end
			end
		end
		amount = amount + Metro_add_amount
	...
	elseif identifier == self.identifiers.armor then
		...
		--Daring Armor
		local liberty_protect = managers.player:body_armor_value("liberty_saves_me", upgrade_level, 0)
		local throwable_add = managers.player:body_armor_value("throwable_add", upgrade_level, 0)
		local medic_armor = managers.player:body_armor_value("medic_armor", upgrade_level, 0)
		local stability_increase = managers.player:body_armor_value("stability_increase", upgrade_level, 0)
		...
		if medic_armor then
			if medic_armor == 1 then
				updated_texts[4].text = updated_texts[4].text .. "\n" .. managers.localization:text("bm_menu_medic_armor_doctor_bag")
			elseif medic_armor == 2 then
				updated_texts[4].text = updated_texts[4].text .. "\n" .. managers.localization:text("bm_menu_medic_armor_first_aid")
			elseif medic_armor == 3 then
				updated_texts[4].text = updated_texts[4].text .. "\n" .. managers.localization:text("bm_menu_medic_armor_all")
			end
		end
		if stability_increase and stability_increase == 1 then
			updated_texts[4].text = updated_texts[4].text .. "\n" .. managers.localization:text("bm_menu_stability_increase")
		end]]
