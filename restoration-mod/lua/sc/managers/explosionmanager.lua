...
function ExplosionManager:_damage_characters(detect_results, params, variant, damage_func_name)
    ...
    local weapon_unit = params.weapon_unit or owner
    ...
    for key, unit in pairs(detect_results.characters_hit) do
        ...
        if alive(unit) and can_damage then
            if unit:character_damage()[damage_func_name] then
                ...
                -- 自定义爆炸衰减 --
                local function calculate_damage_linear(R, min_dmg, max_dmg, d, fix)
                    -- managers.mission._fading_debug_output:script().log("explosive custom: "..R.."-"..min_dmg.."-"..max_dmg.."-"..d, Color.white)
                    -- range200 min10 max27 dis201.3(→200)

                    -- d=math.min(math.max(0,d),R)
                    -- d = (d > r) and r or (d < 0) and 0 or d
                    if d < 0 then
                        d = 0
                    end
                    if d > R then
                    ...
                    min_dmg = (fix and min_dmg / math.sqrt(fix)) or min_dmg
                    max_dmg = (fix and max_dmg / fix) or max_dmg

                    -- managers.mission._fading_debug_output:script().log("result: "..R.."-"..min_dmg.."-"..max_dmg.."-"..d, Color.white)

                    local damage = max_dmg - (max_dmg - min_dmg) * (d / R)
                    -- managers.mission._fading_debug_output:script().log("damage: "..(damage*0.75)..", fix: "..(fix or "None"), Color.red)
                    return damage
                end

                local function calculate_damage_quad(R, min_dmg, max_dmg, d, fix)
                    -- managers.mission._fading_debug_output:script().log("explosive custom: "..R.."-"..min_dmg.."-"..max_dmg.."-"..d, Color.white)
                    ...
                    local damage = (max_dmg - min_dmg) * (1 - ((d ^ 2) / (R ^ 2))) + min_dmg
                    -- managers.mission._fading_debug_output:script().log("damage: "..(damage*0.75)..", fix: "..(fix or "None"), Color.red)
                    return damage
                end

                local function calculate_damage_quad_opp(R, min_dmg, max_dmg, d, fix)
                    -- managers.mission._fading_debug_output:script().log("explosive custom: "..R.."-"..min_dmg.."-"..max_dmg.."-"..d, Color.white)
                    ...
                    local damage = (max_dmg - min_dmg) * ((d - R) ^ 2 / R ^ 2) + min_dmg
                    -- managers.mission._fading_debug_output:script().log("damage: "..(damage*0.75)..", fix: "..(fix or "None"), Color.red)
                    return damage
                end

                local has_exp_func = false
                local weapon_base = weapon_unit and weapon_unit:base()

                if weapon_base then
                    local available_weapon_mods = managers.weapon_factory:get_parts_from_weapon_id(weapon_base._name_id)

                    -- 检测武器改装的exp_func
                    local hit_num_type = nil
                    local fix_mult = 0.75
                    if available_weapon_mods then
                        for _, data in pairs(available_weapon_mods) do
                            for _, part_id in ipairs(data) do
                                if weapon_base:has_part(part_id) then
                                    local factory_id = managers.weapon_factory:get_factory_id_by_weapon_id(
                                        weapon_base._name_id)
                                    local default_blueprint =
                                        managers.weapon_factory:get_default_blueprint_by_factory_id(factory_id)
                                    local part_data = managers.weapon_factory:get_part_data_by_part_id_from_weapon(
                                        part_id, factory_id, default_blueprint)

                                    -- log("currenid = "..factory_id)
                                    -- log("partid = "..part_id)
                                    if part_data then
                                        if part_data.hit_fix and type_checker(part_data.hit_fix) == "number" then
                                            fix_mult = part_data.hit_fix
                                        elseif part_data.hit_fix then
                                            hit_num_type = true
                                        end

                                        if part_data.exp_func then
                                            if part_data.exp_func == "no_falloff" then
                                                if hit_num_type then
                                                    fix_mult = fix_mult * fix_mult
                                                end
                                                -- log("fix:"..fix_mult)
                                                action_data.damage = (part_data.hit_fix and damage / fix_mult) or damage
                                                -- managers.mission._fading_debug_output:script().log("damage: "..action_data.damage)
                                            elseif part_data.exp_func == "linear" then
                                                action_data.damage =
                                                    calculate_damage_linear(range, part_data.min_exp_dmg or 0, damage,
                                                        len, (part_data.hit_fix and fix_mult) or nil)
                                            elseif part_data.exp_func == "quad" then
                                                action_data.damage = calculate_damage_quad(range,
                                                    part_data.min_exp_dmg or 0, damage, len, (part_data.hit_fix and
                                                        fix_mult) or nil)
                                            elseif part_data.exp_func == "quad_op" then
                                                action_data.damage =
                                                    calculate_damage_quad_opp(range, part_data.min_exp_dmg or 0, damage,
                                                        len, (part_data.hit_fix and fix_mult) or nil)
                                            end
                                            ...
                                            break
                                        elseif part_data.hit_fix then
                                            action_data.damage = damage / fix_mult
                                            break
                                        end
                                    end
                                end
                    ...
                    if weapon_tweak and weapon_tweak.hit_fix and type_checker(weapon_tweak.hit_fix) == "number" then
                        fix_mult = weapon_tweak.hit_fix
                    elseif weapon_tweak and weapon_tweak.hit_fix then
                        hit_num_type = true
                    end

                    if not has_exp_func then
                        if weapon_tweak then
                            if weapon_tweak.exp_func then
                                if weapon_tweak.exp_func == "no_falloff" then
                                    if hit_num_type then
                                        fix_mult = fix_mult * fix_mult
                                    end
                                    action_data.damage = (weapon_tweak.hit_fix and damage / fix_mult) or damage
                                elseif weapon_tweak.exp_func == "linear" then
                                    action_data.damage = calculate_damage_linear(range, weapon_tweak.min_exp_dmg or 0,
                                        damage, len, (weapon_tweak.hit_fix and fix_mult) or nil)
                                elseif weapon_tweak.exp_func == "quad" then
                                    action_data.damage = calculate_damage_quad(range, weapon_tweak.min_exp_dmg or 0,
                                        damage, len, (weapon_tweak.hit_fix and fix_mult) or nil)
                                elseif weapon_tweak.exp_func == "quad_op" then
                                    action_data.damage = calculate_damage_quad_opp(range, weapon_tweak.min_exp_dmg or 0,
                                        damage, len, (weapon_tweak.hit_fix and fix_mult) or nil)
                                end
                            end
                        end
                ...
                if dot_data and not dead_now and defense_data and defense_data ~= "friendly_fire" and
                    unit:character_damage().damage_dot then
