...
function CrimeSpreeManager:_setup_mission_lists()
	...
	for index, mission_list in ipairs(tweak_data.crime_spree.missions) do
		...
		for _, mission in ipairs(mission_list) do
			dlc_unlocked = mission and mission.level and (not mission.level.dlc or managers.dlc:is_dlc_unlocked(mission.level.dlc))
			should_hide_unavailable = mission and mission.level and (mission.level.dlc and managers.dlc:should_hide_unavailable(mission.level.dlc) or false)
