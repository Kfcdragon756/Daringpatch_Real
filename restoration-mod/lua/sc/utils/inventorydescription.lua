		...
		for _, stat in pairs(WeaponDescription._stats_shown) do
			...
			if mods_stats[stat.name].index and tweak_stats[stat_name] then
				...
				if stat.name ~= "reload" then
					mods_stats[stat.name].value = (stat.index and index or (tweak_stats and tweak_stats[stat_name] and tweak_stats[stat_name][index] * tweak_data.gui.stats_present_multiplier)) or 1
				end
