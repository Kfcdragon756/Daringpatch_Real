...
local LibertyArmor = restoration.Options:GetValue("HUD/Extra/LibertyArmor")
local LibertyHealth = restoration.Options:GetValue("HUD/Extra/LibertyHealth")
...
function PlayerDamage:init(unit)
	...
	elseif player_manager:has_category_upgrade("player", "dodge_stacking_heal") then
		...
		self._hot_amount = managers.player:upgrade_value("player", "heal_over_time", 0)
		self._hot_amount_multi_2 = self._hot_amount * 1.2
		self.Daring_regen_rate = self._hot_amount_multi_2
	else 
...
function PlayerDamage:damage_fire(attack_data)
	if attack_data.damage < 0 then
		--managers.mission._fading_debug_output:script().log("Check.."..Global.Daring_heal_attch_check, Color.white)
		local heal_damage_d = math.abs(attack_data.damage)
		--managers.mission._fading_debug_output:script().log("Heal = "..heal_damage_d, Color.white)
		self:restore_health(heal_damage_d or 0.05, true)
	end
...
function PlayerDamage:band_aid_health()
	...
	local _, add_health_fak = managers.player:get_equipped_armor_medic()
	self:change_health(tweak_data.upgrades.values.first_aid_kit.heal_amount * self._healing_reduction + add_health_fak)
...
function PlayerDamage:recover_health()
	...
	local doctor_armor = managers.player:get_equipped_armor_medic()
	self:restore_health(tweak_data.upgrades.values.doctor_bag.heal_amount * doctor_armor.health_add) --Initial % heal.
...
function PlayerDamage:_calc_health_damage_no_deflection(attack_data)
	...
	if managers.player:has_category_upgrade("player", "dodge_stacking_heal") and attack_data.damage > 0.0 then --End Rogue health regen.
		--self._damage_to_hot_stack = {}
		self.Daring_regen_rate = self.Daring_regen_rate*0.75
		self.Daring_regen_rate = math.floor(self.Daring_regen_rate*1000+0.5)/1000
		if self.Daring_regen_rate < 0.035 then
			self.Daring_regen_rate = 0.035
		end
	end
...
function PlayerDamage:_calc_health_damage(attack_data)
	...
	--民主护佑
	local function chk_liberty_armor()
		local current_armor = managers.blackmarket:equipped_armor(true, true)
		local liberty_tbl = tweak_data.upgrades.values.player.body_armor.liberty_saves_me
		for i = 1, #liberty_tbl, 1 do
			if liberty_tbl[i] == 1 and current_armor == "level_"..i then
				return true
			end
		end
		return nil
	end
	local rdm_num = math.random()
	if attack_data.damage >= self:get_real_health() and chk_liberty_armor() and rdm_num < tweak_data.upgrades.values.player.body_armor.liberty_saves_me.chance[2] then 
		--managers.mission._fading_debug_output:script().log("Liberty Saves Me!", Color.white)
		if LibertyHealth then
			managers.hud:SetLibertySaveEffect(1.7, Color(0.5, 0.8, 0.5))
		end
		return 0
	end
	...
	if #self._damage_to_hot_stack > 0 then
		repeat
			...
			if not done then
				...
				--managers.mission._fading_debug_output:script().log("Daring_regen_rate is "..self.Daring_regen_rate, Color.white)
				if self.Daring_regen_rate then
					self:restore_health(self.Daring_regen_rate, true)
				else
...
function PlayerDamage:_calc_armor_damage(attack_data)
	...
	local rdm_num = math.random()
	if attack_data.damage >= self:get_real_armor() and chk_liberty_armor() and self:get_real_armor() ~= 0 and rdm_num < tweak_data.upgrades.values.player.body_armor.liberty_saves_me.chance[1] then
		--managers.mission._fading_debug_output:script().log("Liberty Saves Me!", Color.white)
		if LibertyArmor then
			managers.hud:SetLibertySaveEffect(1.7, Color(0.7, 0.8, 0.95))
		end
		return 0
	end
	...
	--破甲无敌
	local player_armor, armor_god_time = managers.blackmarket:equipped_armor_with_break_god_time()
	local player_perk = managers.skilltree:get_specialization_value("current_specialization")
	--managers.mission._fading_debug_output:script().log("my perk is "..player_perk, Color.white)
	--managers.mission._fading_debug_output:script().log("type is "..type(player_perk), Color.white)
