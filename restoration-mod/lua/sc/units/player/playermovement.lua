...
--此处添加，用于修改春哥吼给自己加buff调用的函数
function PlayerMovement:on_morale_boost_for_myself(benefactor_unit)
	if not self.identifier_morale then
		log("self.identifier_morale does not exist, report to LR_Daring")
	end
	...
	if self._morale_boost then
		...
		self._morale_boost = {
			...
			move_speed_bonus = (self.identifier_morale and tweak_data.upgrades.morale_boost_speed_bonus_for_myself) or tweak_data.upgrades.morale_boost_speed_bonus,
			suppression_resistance = tweak_data.upgrades.morale_boost_suppression_resistance,
			reload_speed_bonus = (self.identifier_morale and tweak_data.upgrades.morale_boost_reload_speed_bonus_for_myself) or tweak_data.upgrades.morale_boost_reload_speed_bonus
		}
