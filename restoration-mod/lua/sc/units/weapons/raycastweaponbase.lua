...
local heal_attach = {"dr_wei_MR", "dr_wei_MR_saw"}
local un_fire_weapon = {'saw', 'saw_secondary'}
...
function RaycastWeaponBase:setup(...)
	...
	self._kills_to_head_shot_stacks = self._automatic_kills_to_head_shot_min_stacks
	self._fire_to_head_shot_stacks = self._automatic_fire_to_head_shot_min_stacks
end
...
function RaycastWeaponBase:fire(from_pos, direction, dmg_mul, shoot_player, spread_mul, autohit_mul, suppr_mul, target_unit)
	...
	if run_medical_gun then
		for _,b in ipairs(heal_attach) do
			if table.contains(self._blueprint or {}, b) then
				heal_attach_match = true
				attachment_id = b
				break
			end
		end
	end
	if not heal_attach_match then
		--managers.mission._fading_debug_output:script().log("blueprint not found", Color.white)
	...
	local raycast
	local heal_damage = 0
	if attachment_id == 'dr_wei_MR' then
		heal_damage = -0.5
	end
	if weapon_hold == 'new_raging_bull' then
		heal_damage = heal_damage - 0.1
	end
	if run_medical_gun and managers.player and managers.player:player_unit() then
		camera = managers.player:player_unit():camera()
		from = camera:position()
		to = camera:position() + camera:rotation():y() * 50000
		raycast = World:raycast("ray", from, to, "slot_mask", managers.slot:get_mask("all_criminals", "bullet_blank_impact_targets"))
		if raycast and raycast.unit then
			local peer_id = managers.criminals:character_peer_id_by_unit(raycast.unit)
			local peer = peer_id and managers.network._session:peer(peer_id)
			
			if peer then
				--managers.mission._fading_debug_output:script().log("heal = "..heal_damage, Color.white)
				local healing_message = {
					attchment = attachment_id,
					heal_value = math.abs(heal_damage),
					weapon_holding = weapon_hold
				}
				LuaNetworking:SendToPeer(peer_id, 'Resmod_Daring_heal_num', json.encode(healing_message))
				
				peer:send("sync_friendly_fire_damage", peer_id, peer:unit(), heal_damage, "fire")
			end
		end
	end
	--===========================结束============================-
...
function RaycastWeaponBase:KFC_recategories()
	return self:weapon_tweak_data().recategorize
end

function RaycastWeaponBase:KFC_is_recategory(...)
	local arg = {
		...
	}
	local recategories = self:KFC_recategories()

	if not recategories then
		return false
	end

	for i = 1, #arg do
		if table.contains(recategories, arg[i]) then
			return true
		end
	end
...
--由于此处的fire函数对电锯、霰弹枪、下挂等武器不生效，所以此段可以暂且用于这类武器
Hooks:PostHook(RaycastWeaponBase, "weapon_fire_rate", "Daring_sc_heal_player_ray_saw", function(self)
	...
	if not heal_attach_match or not un_fire_weapon_check then
		--managers.mission._fading_debug_output:script().log("blueprint not found", Color.white)
		--log('blueprint not found')
		return
	end
	...
	if attachment_id == 'dr_wei_MR' then
		heal_damage = -0.5
	elseif attachment_id == 'dr_wei_MR_saw' then
		heal_damage = -0.1
	end
	if weapon_hold == 'saw' or weapon_hold == 'saw_secondary' then
		heal_damage = heal_damage + 0.02
	end
	if managers.player:player_unit() then
		...
		if raycast and raycast.unit then
			...
			if peer then
				...
				LuaNetworking:SendToPeer(peer_id, 'Resmod_Daring_heal_num', json.encode(healing_message))
				--[[if is_pro then
					managers.mission._fading_debug_output:script().log("is_pro", Color.white)
				end--]]
