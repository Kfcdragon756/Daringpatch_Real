...
Hooks:PostHook(NewRaycastWeaponBase, "init", "ResExtraSkills", function(self)
	...
	for _, category in ipairs(self:categories()) do
		...
		if managers.player:has_category_upgrade(category, "automatic_kills_to_head_shot") then
			self._automatic_kills_to_head_shot_max_stacks = managers.player:upgrade_value(category, "automatic_kills_to_head_shot")[1]
			self._automatic_kills_to_head_shot_min_stacks = managers.player:upgrade_value(category, "automatic_kills_to_head_shot")[2]
			self._automatic_kills_to_head_shot_dmg_mult = managers.player:upgrade_value(category, "automatic_kills_to_head_shot")[3]
		end
		
		if managers.player:has_category_upgrade(category, "automatic_fire_to_head_shot") then
			self._automatic_fire_to_head_shot_max_stacks = managers.player:upgrade_value(category, "automatic_fire_to_head_shot")[1]
			self._automatic_fire_to_head_shot_min_stacks = managers.player:upgrade_value(category, "automatic_fire_to_head_shot")[2]
			self._automatic_fire_to_head_shot_dmg_mult = managers.player:upgrade_value(category, "automatic_fire_to_head_shot")[3]
			self._automatic_fire_to_head_shot_punishment = managers.player:upgrade_value(category, "automatic_fire_to_head_shot")[4]
		end
	end
...
function NewRaycastWeaponBase:recoil_multiplier(...)
	...
	if current_state then
		...
		if user_unit:movement()._state_data.ducking then
			...
			if is_moving then
				local recoil = managers.player:get_equipped_armor_recoil()[1]
				mult = mult * (1+(1-recoil))
			else
				local recoil = managers.player:get_equipped_armor_recoil()[2]
				mult = mult * (1+(1-recoil))
			end
		end
	end
...
local Daring_Target_Enable = restoration.Options:GetValue("OTHER/DaringTagetingChange")
function NewRaycastWeaponBase:check_highlight_unit(unit)
	...
	if not unit or not alive(unit) or not unit:base() then
		return
	end
	...
	local ignore_enemy = {'gangster', 'fbi_heavy_swat', 'fbi_swat', 'city_swat', 'heavy_swat', 'swat', 'city_swat_guard', 'weekend',
							'cop', 'cop_forest', 'security', 'hrt', 'zeal_heavy_swat', 'zeal_swat'}
	local light_swats = {'swat', 'heavy_swat', 'fbi_swat', 'city_swat', 'zeal_swat', 'hrt'}
	local heavy_swats = {'city_swat_guard', 'fbi_heavy_swat', 'heavy_swat', 'zeal_heavy_swat'}
	...
	if Daring_Target_Enable then
		is_enemy_in_cool_state = managers.enemy and managers.enemy:is_enemy(unit)
	end



	if Daring_Target_Enable then
		if Holding_LightAR then
			if not is_enemy_in_cool_state then-- and not unit:base().can_be_marked then
				return
			end
		elseif Holding_HeavyAR then
				for _,v in ipairs(light_swats) do
					if unit_name_id and unit_name_id == v then
						is_ignore_enemy = true
						break
					end
				end
				if is_ignore_enemy then
					return
				end
				if not is_enemy_in_cool_state then
					return
				end
		else
			for _,v in ipairs(ignore_enemy) do
				if unit_name_id and unit_name_id == v then
					is_ignore_enemy = true
					break
				end
...
function NewRaycastWeaponBase:get_automatic_kills_to_head_shot_stacks()
	if self:is_category("smg", "lmg") and self._fire_mode == ids_auto then
		local stacks = 0
		if managers.player:has_category_upgrade("smg", "automatic_kills_to_head_shot") then
			stacks = stacks + (self._kills_to_head_shot_stacks or 0)
		end
		
		if managers.player:has_category_upgrade("smg", "automatic_fire_to_head_shot") then
			stacks = stacks + (self._fire_to_head_shot_stacks or 0)
		end
